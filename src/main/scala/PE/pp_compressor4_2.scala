...
class pp_compressor4_2 extends Module{
    val input  = IO(new pp_compressor4_2InputBundle(16))
    val output = IO(new pp_compressor4_2OutputBundle(16))

    val c_temp = Wire(Vec(17, Bool()))
    c_temp(0) := false.B

    val s_comb     = Wire(Vec(16, Bool()))
    val c_comb     = Wire(Vec(16, Bool()))
    val c_comb_int = Wire(UInt(16.W))
    ...
    output.C   := Cat(c_comb_int(14,0), 0.U(1.W))
}